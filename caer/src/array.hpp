...
#ifndef CAER_NUMPYPP_ARRAY_HPP_INCLUDE_GUARD_LPC_
#define CAER_NUMPYPP_ARRAY_HPP_INCLUDE_GUARD_LPC_
...
namespace numpy {
    ...
    struct position {
        ...
        bool operator == (const position& other) { 
            return !std::memcmp(this->position_,other.position_,sizeof(this->position_[0])*this->nd_); 
        }
            ...
            array_base(const array_base<BaseType>& other)
                :array_(other.array_)
                {
                    if (sizeof(BaseType) != PyArray_ITEMSIZE(array_)) {
                        std::cerr << "CAER:" << __PRETTY_FUNCTION__ << " mix up of array types"
    ...
    inline
    bool is_carray(PyArrayObject* a) { 
        return are_arrays(a) && PyArray_ISCARRAY(a); 
    }

} // namespace numpy

#endif // CAER_NUMPYPP_ARRAY_HPP_INCLUDE_GUARD_LPC_
